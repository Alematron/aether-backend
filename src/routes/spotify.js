...
async function refreshSpotifyToken(refreshToken, handle) {
  try {
    const creds = Buffer.from(
      process.env.SPOTIFY_CLIENT_ID + ":" + process.env.SPOTIFY_CLIENT_SECRET
    ).toString("base64");
    ...
    await query(
      "UPDATE users SET spotify_access_token = $1, spotify_token_expires = NOW() + INTERVAL '1 hour' WHERE handle = $2",
      [tokens.access_token, handle]
    );
